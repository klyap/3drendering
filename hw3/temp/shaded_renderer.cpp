...
void create_obj_template(Obj &obj, char * filename){
  // Set up structs
  ...
  for (auto &f_i : f){
    vertex_buffer.push_back(v[f_i.x]);
    vertex_buffer.push_back(v[f_i.y]);
    vertex_buffer.push_back(v[f_i.z]);
  }

  // Construct normal buffer
  vector<Vertex> normal_buffer;
  for (auto &fn_i : fn){
    vertex_buffer.push_back(vn[fn_i.x]);
    vertex_buffer.push_back(vn[fn_i.y]);
    vertex_buffer.push_back(vn[fn_i.z]);
  }
