...
struct Obj {
  char * filename;
  std::vector<Vertex> v; // v buffer
  std::vector <Face> f;
  std::vector<Vertex> vn; // normal buffer
  std::vector <Face> fn;
  std::vector <Vertex> plain_v;
  Material material;
...
//extern float h; // time step for implicit fairing
...
void parse_points(Obj &obj, char * filename, vector<HEV*> *hevs, vector<HEF*> *hefs, Mesh_Data *mesh);

void parse_transform(istringstream &infile, vector<GeoTransform> &all);

void create_originals(string originals_text, vector<HEV*> *hevs, vector<HEF*> *hefs, Mesh_Data *mesh);
