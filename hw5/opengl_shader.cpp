...
vector<HEV*> *hevs_new;// = new vector<HEV*>;
...
Eigen::SparseMatrix<double> build_F_operator(
  std::vector<HEV*> *vertices, float h)
{
    ...
    for( int i = 1; i < vertices->size(); ++i )
    {
        ...
        // Precompute area of all 6 adjacent faces to this v_i
        float A = 0;
        do{
          HEF *f = he->face;
        ...
        /* Compute the rest of the Laplacian equation */
        he = vertices->at(i)->out;
        float delta_ii = 0; // sum of all adjacent delta_ij
        do // iterate over all vertices adjacent to v_i
        {
            ...
            if (A < 0.0000000001){
              ...
              HEV *v1_v = he->vertex; // v_i
              HEV *v2_v = he->next->vertex; // v_j
              HEV *v3_v = he->next->next->vertex; // alpha angle
              HE *he2 = he->flip;
              HEV *v1_v2 = he2->vertex; // v_i
              HEV *v2_v2 = he2->next->vertex; // v_j
              HEV *v3_v2 = he2->next->next->vertex; // beta angle

              // Convert HEV's to Eigen
              Vector3d v1(v1_v->x, v1_v->y, v1_v->z); // v_i
              Vector3d v2(v2_v->x, v2_v->y, v2_v->z); // v_j
              Vector3d v3(v3_v->x, v3_v->y, v3_v->z);

              Vector3d v12(v1_v2->x, v1_v2->y, v1_v2->z); // v_i
              Vector3d v22(v2_v2->x, v2_v2->y, v2_v2->z); // v_j
              Vector3d v32(v3_v2->x, v3_v2->y, v3_v2->z);

              /* Compute alpha angle */
              Vector3d a = v1 - v2;
              Vector3d b = v3 - v2;
              float cot_alpha_j = a.dot(b) / (a.cross(b).norm());

              /* Compute beta angle */
              Vector3d c = v12 - v22;
              Vector3d d = v32 - v22;
              float cot_beta_j = c.dot(d) / (c.cross(d).norm());

              // 1/(2A) * (cot(alpha_j) + cot(beta_j)) * (x_j - x_i)
              delta_ij = 1 / (2 * A) * (cot_alpha_j + cot_beta_j);

              /* fill the j-th slot of row i of our F matrix */
              F.insert( i-1, j-1 ) = delta_ij;
            }

            /* Update delta_ii sum */
            delta_ii -= delta_ij;

            he = he->flip->next; // iterate to next adjacent vertex
        }
        while( he != vertices->at(i)->out );

        /* fill the i-th slot of row i of our F matrix */
        F.insert( i-1, i-1 ) = delta_ii;
    }

    F.makeCompressed(); // optional; tells Eigen to more efficiently store our sparse matrix
    Eigen::MatrixXd id = Eigen::MatrixXd::Identity(num_vertices, num_vertices);
    return id.sparseView() - h * F;;
}

// function to solve Equation 4
void solve(
  std::vector<HEV*> *vertices,
  std::vector<HEV*> *smooth_vertices)
{
    // Set up new smooth vertices
    smooth_vertices->push_back(NULL);
    for (int i = 1; i < vertices->size(); i++){
      HEV *placeholder = new HEV;
      smooth_vertices->push_back(placeholder);
    }
    ...
    // Calculate phi for all x, y and z coords of vertices
    // And directly update each vertex in obj
    // dim = 0 -> x dimension
    // dim = 1 -> y dimension
    // dim = 2 -> z dimension
    for (int dim = 0; dim < 3; dim++){
      ...
      for( int i = 0; i < num_vertices; ++i ){ // since vertices is 1 indexed
        if (dim == 0){
          rho_vector(i) = vertices->at(i + 1)->x;
        } else if (dim == 1){
          rho_vector(i) = vertices->at(i + 1)->y;
        } else if (dim == 2){
          rho_vector(i) = vertices->at(i + 1)->z;
        }
      } // end of generating rho

      // Have Eigen solve for our phi_vector
      Eigen::VectorXd phi_vector( num_vertices );
      phi_vector = solver.solve( rho_vector );

      // phi is now the smoothed out dimension of a vertex
      // Update that dimension for all vertices
      for ( int i = 0; i < num_vertices; ++i ){
        if (dim == 0){
          smooth_vertices->at(i + 1)->x = phi_vector(i);
        } else if (dim == 1){
          smooth_vertices->at(i + 1)->y = phi_vector(i);
        } else if (dim == 2){
          smooth_vertices->at(i + 1)->z = phi_vector(i);
        }
      }
    } // End of finding phi for all 3 dims
}
...
void init(string textfile)
{
    ...
    hevs = new vector<HEV*>;
    hefs = new vector<HEF*>;
    create_originals(originals_text, hevs, hefs, mesh);
...
void key_pressed(unsigned char key, int x, int y)
{
    ...
    /* If 'e' is pressed, apply implicit fairing with the inputted timestep
     */
    else if(key == 'e')
    {
        // TODO:  Recalculate vertex positions and corresponding normals
        if (hevs->size() > 0){
          hevs_new = new vector<HEV*>;
          solve(hevs, hevs_new);

          // Create new list of vertices
          for (auto &obj : instances){
            for (int i = 1; i < obj.plain_v.size(); i++){
              obj.plain_v[i].x = hevs_new->at(i)->x;
              obj.plain_v[i].y = hevs_new->at(i)->y;
              obj.plain_v[i].z = hevs_new->at(i)->z;
            }
            ...
            calculate_normal(vn, hevs_new, hefs);
            ...
            for (auto &f_i : obj.f){
              vertex_buffer.push_back(obj.plain_v[f_i.idx1]);
              vertex_buffer.push_back(obj.plain_v[f_i.idx2]);
              vertex_buffer.push_back(obj.plain_v[f_i.idx3]);
            }
...
{
    if (argc < 5){
        cout << "Usage: ./opengl_shader [scene_description_file.txt] [xres] [yres] [h]"
          << endl;
        return 0;
    }
    // Use input arguments
