...
void display(void)
{
    ...
    for(int i = 0; i < vertices.size(); i++)
    {
         ...
         */
         ...
         //L = 1/2 * m * ( v_x^2 + v_y^2 ) - U( x, y )
         float x_k = vertices[i].x;
         float y_k = vertices[i].y;
         float p_xk = vertices[i].vx;
         float p_yk = vertices[i].vy;
         float m = vertices[i].mass;
         float dt = tstep;

         vertices[i].x =
            (dt * dt * vertices[i].fx)/m + (dt * p_xk)/m + x_k;
         vertices[i].y =
            (dt * dt * vertices[i].fy)/m + (dt * p_yk)/m + y_k;

        // velocity = momentum / mass
        vertices[i].vx =   (m * (-x_k + vertices[i].x)/dt) / m;
        vertices[i].vy =  (m * (-y_k + vertices[i].y)/dt) / m;
